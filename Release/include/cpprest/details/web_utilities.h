...
public:
	...
	enum web_proxy_mode_internal { use_default_, use_auto_discovery_, disabled_, ie_settings_, user_provided_ };

	enum web_proxy_mode { use_default = use_default_, use_auto_discovery = use_auto_discovery_, disabled = disabled_, ie_settings = ie_settings_ };

    /// <summary>
    /// Constructs a proxy with the default settings.
    /// </summary>
    web_proxy() : m_address(_XPLATSTR("")), m_mode(ie_settings_) {}
	...
	/// <summary>
	/// Checks if the current user IE had initialized proxy settings, then IE proxy settings is to be used.
	/// </summary>
	/// <returns>True if auto discovery enabled, false otherwise.</returns>
	bool is_ie_settings() const { return m_mode == ie_settings_; }

	web_proxy_mode_internal internal_mode() const
    {
		return m_mode;
    }
