...
datetime __cdecl datetime::from_string(const utility::string_t& dateString, date_format format)
{
    ...
    static boost::mutex env_var_lock;//Используется только для андроида, так что пофиг, что здесь локальный static
    {
...
endianness conversions::check_byte_order_mark(const utf16string& str)
{
	if (str.empty())
	{
		return unknown;
...
std::wstring conversions::convert_string_t_to_wstring(string_t src)
{
#ifdef _UTF16_STRINGS
	return /*convert_utf16_to_utf16*/(std::move(src));
#else
	return convert_utf8_to_utf16(std::move(src));
#endif
}

std::string conversions::convert_utf16le_to_utf8(utf16string src, bool erase_bom)
{
...
utility::string_t conversions::convert_utf16le_to_string_t(utf16string src, bool erase_bom)
{
	if (erase_bom && !src.empty())
	{
		src.erase(0, 1);
...
utf16string conversions::big_endian_to_little_endian(utf16string src, bool erase_bom)
{
	if (erase_bom && !src.empty())
	{
		src.erase(0, 1);
...
std::string conversions::convert_utf16be_to_utf8(utf16string src, bool erase_bom)
{
	return utf16_to_utf8(big_endian_to_little_endian(std::move(src), erase_bom));
}

utf16string conversions::convert_utf16be_to_utf16le(utf16string src, bool erase_bom)
{
	return big_endian_to_little_endian(std::move(src), erase_bom);
}

utility::string_t conversions::convert_utf16be_to_string_t(utf16string src, bool erase_bom)
{
#ifdef _UTF16_STRINGS
...
std::string conversions::convert_utf16_to_utf8(utf16string src)
{
...
utf16string conversions::convert_utf16_to_utf16(utf16string src)
{
	const endianness endian = check_byte_order_mark(src);
	switch (endian)
	{
...
utility::string_t conversions::convert_utf16_to_string_t(utf16string src)
{
#ifdef _UTF16_STRINGS
	return convert_utf16_to_utf16(std::move(src));
#else
